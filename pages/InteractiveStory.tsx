...
import { StoryNode, AIVoices, Project } from '../types';
...
const InteractiveStory: React.FC = () => {
  ...
  useEffect(() => {
     if (location.state?.project) {
        ...
        if (proj.data?.storyNodes) {
           setNodes(proj.data.storyNodes);
           setStoryPremise(proj.title || '');
        }
     }
  }, [location]);
  ...
  const handleSaveProject = async () => {
     const title = storyPremise || "Untitled Story";
     ...
     try {
        ...
        showToast("Story saved to database!", "success");
     } catch (e) {
        console.error(e);
       ...
       <div className="h-16 border-b border-gray-800 bg-gray-900/80 backdrop-blur px-6 flex justify-between items-center z-20">
          ...
          <div className="flex items-center gap-2">
             ...
             <button 
                ...
                className="bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg font-bold flex items-center ml-4 transition-colors"
             >
                <Save size={18} className="mr-2" /> Save
             </button>
             <button 
                onClick={handleStartPlay}
                className="bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded-lg font-bold flex items-center shadow-lg transition-transform hover:scale-105"
             >
