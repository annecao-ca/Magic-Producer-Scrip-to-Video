...
import { Plus, Video, FileText, Clock, ArrowUpRight, Folder, Trash2, Star, Trophy, Award, PlayCircle, Zap, Sparkles, Building, TrendingUp, Briefcase, GitBranch, Headphones } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const getProjectIcon = (type: string) => {
     switch(type) {
        ...
        case 'interactive': return <GitBranch size={14} />;
        case 'podcast': return <Headphones size={14} />;
        case 'magic': return <Sparkles size={14} />;
  ...
  const getProjectColor = (type: string) => {
     switch(type) {
        case 'video': return 'bg-brand-900/10 border-brand-500/20 text-brand-500';
        case 'script': return 'bg-blue-900/10 border-blue-500/20 text-blue-500';
        case 'interactive': return 'bg-purple-900/10 border-purple-500/20 text-purple-500';
        case 'podcast': return 'bg-pink-900/10 border-pink-500/20 text-pink-500';
        case 'magic': return 'bg-yellow-900/10 border-yellow-500/20 text-yellow-500';
                              ...
                              <tr key={project.id} className="hover:bg-gray-800/50 transition-colors group">
                                 <td className="px-6 py-4 font-medium text-white">
                                    <div className="flex items-center">
                                       <div className={`w-8 h-8 rounded-lg flex items-center justify-center mr-3 border ${getProjectColor(project.type)}`}>
