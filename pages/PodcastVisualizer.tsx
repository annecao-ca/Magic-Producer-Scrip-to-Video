...
import { Headphones, Music, Mic, Video, Upload, Play, Pause, Download, Image as ImageIcon, RefreshCw, Loader2, Users, X, Save } from 'lucide-react';
import { useLanguage } from '../contexts/LanguageContext';
import { storageService } from '../services/storageService';
import { SavedCharacter, Project } from '../types';
...
const PodcastVisualizer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (location.state?.project) {
       ...
       if (proj.data?.podcastData) {
          const d = proj.data.podcastData;
          setCaption(d.caption || '');
          setMode(d.mode || 'spectrum');
          if(d.backgroundUrl) setBackgroundUrl(d.backgroundUrl);
          if(d.generatedVideoUrl) setGeneratedVideoUrl(d.generatedVideoUrl);
          // Audio restoration would require blob storage or re-upload
          if (d.audioBlobBase64) {
             // Decode base64 to blob
             try {
                const byteCharacters = atob(d.audioBlobBase64);
  ...
  const handleSaveProject = async () => {
     // Convert audio url to blob base64 if possible
     ...
     if (audioUrl) {
        ...
        } catch(e) { console.warn("Could not save audio blob"); }
     }

     const project: Project = {
        id: currentProjectId || Date.now().toString(),
        title: caption || "Podcast Project",
        type: 'podcast',
        status: 'completed',
     ...
     try {
        ...
        showToast("Podcast project saved!", "success");
     } catch(e) {
        console.error(e);
    ...
    <div className="max-w-6xl mx-auto pb-20 animate-fadeIn">
      <div className="mb-8 flex justify-between items-center">
        ...
        <button 
           ...
           className="bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 rounded-lg font-bold flex items-center shadow-lg"
        >
